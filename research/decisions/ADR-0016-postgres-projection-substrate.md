...
## Status
Accepted for the prototype (`ptr-pg`, feature `postgres-backend`). L004 passed on PostgreSQL 18 (2026-09-26; five seeds, no divergence). Production use is still gated on the `relational-substrate` evaluation and on L004 runs for PostgreSQL 16 and 17 and with readers concurrent with the projector.
