...
## Consequences
The checkpoint is not trusted as a Pod backend until experiment R003 shows measurable improvement over the unmodified base model on held-out code from the user's own repos. Serving-backend selection for this Pod stays gated on the pre-existing `inference-serving` evaluation resolving; training-backend selection is gated on the new evaluation resolving separately. `training/` keeps `dry-run` as the default; `--execute` still refuses a real backend until one is selected.
