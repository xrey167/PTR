...
A0's own mechanisms. Item 1.2 is done in part: as an A0-internal study on a
synthetic set only (the two imported hash-verified bundles were not used), not as
the matched baseline comparison, which still waits on 1.6 (owner decision O2):

| Item | What changed | Where |
|---|---|---|
| 1.1 | A0 gains four switches beside `latent_steps` (typed attention, typed query, latent nonlinearity, frozen router). Each has a test that it changes the forward pass; the typed bias, the gelu and the frozen router also have tests that they remove exactly their mechanism, and the typed query is checked jointly with the other metadata reads. `ablations.toml` maps every ablation onto a study arm, and a test keeps it, the study config and the binary's arm table in agreement. `no-verifier-head` stays "not tested": A0 has no such module | `model/burn-a0/README.md` "Ablation switches", `scripts/tests/test_a0_ablation_config.py` |
...
| 1.3 | `run_experiment.py aggregate` turns per-seed records into one aggregate. It refuses mixed commits, dirty worktrees and undeclared seeds, and for failed runs, missing seeds or non-finite values it writes an aggregate marked `incomplete` and exits 1. The M001–M004 manifests have `a0_*` entrypoints; `entrypoint` and `metrics.json` stay reserved for the baseline comparison | `scripts/tests/test_run_experiment.py` |
| 1.4 | `hardware/a0-cpu-4core.toml` holds measured values, and every run record embeds the profile's contents, the host and rustc. The M001–M004 manifests name it; the baseline comparison must name its own host's profile | `scripts/tests/test_run_experiment.py` |
