...
def main() -> None:
    ...
    try:
        revision = experiment_records.source_revision("L003", manifest, named, ROOT, HERE)
        ...
        mutations = experiment_records.mutation_evidence(
            "L003", RESULTS / "mutations.json", BENCHMARK, ROOT, HERE
        )
    except experiment_records.ProvenanceError as error:
        raise SystemExit(f"L003: refusing to aggregate: {error}")
    ...
    print(
        f"L003 verdict={verdict} hard_pass={hard_pass} coverage_ok={coverage_ok} cases={totals['cases']} "
