...
Every JSON object a run printed on its own line is a row: its string fields name it (for example `{"arm": "typed", "split": "ood"}`) and its numeric fields are summarized across seeds (n, mean, sample standard deviation, min, max, and each seed's value). The aggregate is written as a new immutable `aggregate-<timestamp>-<entrypoint>.json` that lists the SHA-256 of every record it read. It refuses records from more than one commit (choose one with `--git-sha`), records from a dirty or unrecorded worktree (unless `--allow-dirty`, which it records), two completed runs of one seed, and a row printed twice by one run. Failed runs and declared seeds with no completed run are listed, never dropped; they make the aggregate `incomplete` and the command exit 1.
