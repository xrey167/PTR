...
**Code footprint:** 3 Rust source files · 681 nonblank source lines · 1 integration-test files · 6 `#[test]` markers
...
- Snapshot payloads are carried unread, and the test carries a real PTRCS001 compacted snapshot end to end: it is recorded by the leader, arrives byte for byte at a member the leader cannot replay to, and a runtime restores from exactly those bytes
...
- a real PTRCS001 snapshot travels as the payload to a restarted member the leader compacted past, arrives byte for byte, and a runtime restores the state it described from exactly those bytes
