...
**Code footprint:** 9 Rust source files · 4441 nonblank source lines · 12 integration-test files · 102 `#[test]` markers
...
- FileRaftStorage keeps a fencing token — the highest term it has ever accepted — in the state file, re-read from disk before every write, and refuses a writer at a lower term with PTR_RAFT_FENCED so a process partitioned from its peers but not from their storage, or resumed from a stale image, cannot write beneath the group
