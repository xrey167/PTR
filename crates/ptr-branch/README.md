...
- The reason behind a verification-decided triage (status, level, hard-finding codes) and behind a certification refusal (conflicting keys, changed lifecycle targets), stored as structured fields with the triage row and the outcome rather than as free text
