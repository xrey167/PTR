...
impl Writer {
    fn raw(&mut self, value: &[u8]) -> Result<(), SemanticError> {
        if value.len() > MAX_DELTA_BYTES.saturating_sub(self.0.len()) {
    ...
    fn key(&mut self, value: &str) -> Result<(), SemanticError> {
        key(value)?;
...
impl SemanticDelta {
    pub fn encode(&self) -> Result<Vec<u8>, SemanticError> {
        if self
            .removals
            .iter()
            .any(|key| self.upserts.contains_key(key) || self.dependencies.contains_key(key))
        {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, SemanticError> {
        ...
        if reader.take(8)? != MAGIC {
            return Err(SemanticError::InvalidEncoding);
        }
        ...
        for _ in 0..reader.count()? {
            ...
            let value = match reader.take(1)?[0] {
                ...
                1 => SemanticValue::Payload(SemanticPayload {
                    type_id: TypeId(reader.key()?),
                    source: reader.key()?,
                    bytes: reader.bytes()?.to_vec(),
                }),
                _ => return Err(SemanticError::InvalidEncoding),
            };
            if delta.upserts.insert(name, value).is_some() {
                return Err(SemanticError::InvalidEncoding);
            }
        }
        for _ in 0..reader.count()? {
            if !delta.removals.insert(reader.key()?) {
                return Err(SemanticError::InvalidEncoding);
            }
        }
        for _ in 0..reader.count()? {
            ...
            for _ in 0..reader.count()? {
                if !inputs.insert(reader.key()?) {
                    return Err(SemanticError::InvalidEncoding);
                }
            }
            if delta.dependencies.insert(name, inputs).is_some() {
                return Err(SemanticError::InvalidEncoding);
            }
        }
        ...
        if reader.offset != bytes.len() || delta.encode()? != bytes {
            return Err(SemanticError::InvalidEncoding);
        }
        Ok(delta)
    }
...
impl<'a> Reader<'a> {
    fn take(&mut self, length: usize) -> Result<&'a [u8], SemanticError> {
        let end = self
            .offset
            .checked_add(length)
            .ok_or(SemanticError::InvalidEncoding)?;
        let value = self
            .bytes
            .get(self.offset..end)
            .ok_or(SemanticError::InvalidEncoding)?;
    ...
    fn count(&mut self) -> Result<usize, SemanticError> {
        let count = self.u32()?;
        if count > MAX_DELTA_ITEMS || count > (self.bytes.len() - self.offset) / 4 {
            return Err(SemanticError::LimitExceeded);
        }
        Ok(count)
    }
    ...
    fn string(&mut self) -> Result<String, SemanticError> {
        std::str::from_utf8(self.bytes()?)
            .map(str::to_owned)
            .map_err(|_| SemanticError::InvalidEncoding)
    }
    fn key(&mut self) -> Result<String, SemanticError> {
