...
fn local_change_invalidates_only_dependency_closure() {
    ...
    setup
        .dependencies
        .insert("claim:x".into(), ["input:b".into()].into());
    setup
        .dependencies
        .insert("plan:p".into(), ["claim:x".into()].into());
    setup
        .dependencies
        .insert("unrelated".into(), ["input:a".into()].into());
    host.apply_delta(setup).unwrap();
    let (_, affected) = host.apply_delta(delta("input:b", "new")).unwrap();
    assert_eq!(
        affected,
        ["input:b".into(), "claim:x".into(), "plan:p".into()].into()
    );
}
...
fn payload_bytes_type_and_source_are_all_revision_significant() {
    ...
    let mut p = SemanticPayload {
        type_id: TypeId::from("bytes"),
        source: "pod:a".into(),
        bytes: (0..=255).collect(),
    };
    for expected in 1..=4 {
        match expected {
            2 => p.bytes[0] = 17,
            3 => p.source = "pod:b".into(),
            4 => p.type_id = "other".into(),
            _ => {}
        }
...
fn edits_and_removals_evict_transitive_derivations_but_not_unrelated_values() {
    for remove in [false, true] {
        ...
        if remove {
            assert!(matches!(
                host.apply_delta(delta("derived", "old")),
...
fn explicit_recomputation_and_dependency_replacement_have_deterministic_semantics() {
    ...
    replacement
        .dependencies
        .insert("derived".into(), ["unrelated".into()].into());
    host.apply_delta(replacement).unwrap();
    assert_eq!(host.revision(), Revision(3));
    assert_eq!(host.snapshot().get("derived"), None);
    assert_eq!(
        host.snapshot().inputs("derived").collect::<Vec<_>>(),
        vec!["unrelated"]
    );
    host.apply_delta(delta("derived", "from new input"))
        .unwrap();
...
fn missing_inputs_cycles_and_conflicting_operations_are_atomic_rejections() {
    ...
    missing
        .dependencies
        .insert("new".into(), ["missing".into()].into());
    let mut cycle = SemanticDelta::default();
    cycle
        .dependencies
        .insert("source".into(), ["plan".into()].into());
...
fn codec_has_a_golden_empty_encoding_and_rejects_all_truncated_prefixes() {
    ...
    for end in 0..bytes.len() {
        assert!(
            SemanticDelta::decode(&bytes[..end]).is_err(),
            "prefix {end}"
        );
    }
...
fn seeded_transaction_streams_replay_identical_values_dependencies_and_revisions() {
    for seed in [17_u64, 29, 43, 71, 101] {
        ...
        for step in 0..100 {
            ...
            let encoded = d.encode().unwrap();
            assert_eq!(
                writer.apply_delta(d).unwrap(),
                replay
                    .apply_delta(SemanticDelta::decode(&encoded).unwrap())
                    .unwrap()
            );
            let a = writer.snapshot();
            let b = replay.snapshot();
