...
pub const MAX_COMPACTED_BYTES: usize = HEADER + 2 * MAX_SECTION_BYTES + DIGEST_BYTES;
...
impl CompactedError {
    ...
    pub fn code(self) -> &'static str {
        match self {
            ...
            Self::ReservedField => "PTR_COMPACTED_RESERVED_FIELD",
...
impl PtrRuntime {
    ...
    pub fn export_compacted_snapshot(&self) -> Result<CompactedSnapshot, RuntimeError> {
        ...
        if semantic.len() > MAX_SECTION_BYTES || lifecycle.len() > MAX_SECTION_BYTES {
            return Err(invalid(CompactedError::SizeLimit));
        }
        let mut bytes =
            Vec::with_capacity(HEADER + semantic.len() + lifecycle.len() + DIGEST_BYTES);
    ...
    pub fn restore_compacted(
        ...
        let (semantic, lifecycle) = compacted_sections(bytes, trusted)?;
...
fn compacted_sections(
    ...
    if bytes[72..80] != [0; 8] {
        return Err(invalid(CompactedError::ReservedField));
    }
    if semantic_len > MAX_SECTION_BYTES as u64 || lifecycle_len > MAX_SECTION_BYTES as u64 {
        return Err(invalid(CompactedError::SizeLimit));
    }
    let body = (bytes.len() - HEADER - DIGEST_BYTES) as u64;
    if semantic_len.checked_add(lifecycle_len) != Some(body) {
        return Err(invalid(CompactedError::LengthMismatch));
    }
    ...
    let split = HEADER + semantic_len as usize;
    Ok((&bytes[HEADER..split], &bytes[split..end]))
}
