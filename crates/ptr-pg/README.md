...
**Code footprint:** 14 Rust source files · 3032 nonblank source lines · 1 integration-test files · 16 `#[test]` markers
...
- Tombstones delete the revoked generation's fast-memory writes and the checkpoints that folded them in the projector's transaction; appends are refused for inadmissible sources
- Platform metrics compiled from ptr-analytics definitions to SQL over the work schema only
- Capability probe that never creates an extension; refusal of non-loopback hosts because the build links no TLS connector
